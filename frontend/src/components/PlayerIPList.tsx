            ...
            player.ips.map((ip, index) => (
              <tr key={`${player.username}-${index}`}>
                ...
                <td className="protocol-cell">
                  <span className={`protocol-badge ${ip.protocol.toLowerCase()}`}>
                    {ip.protocol}
                  </span>
                </td>
                <td className="timestamp-cell">{formatLastSeen(ip.lastSeen)}</td>
                <td className="time-since-cell">{formatTimeSince(ip.lastSeen)}</td>
              </tr>
            ))
          ))}
        </tbody>
